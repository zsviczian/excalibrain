...
export class Node {
  ...
  public getDisplayText(limitLength?: number, refresh: boolean = false): string {
    if (this.displayText && !refresh) {
      console.log(`displayTextExist: ${this.displayText}`);
  ...
  public getLabelSize(limitLength:number): Dimensions {
    const ea = this.setEA();
    const text = this.getDisplayText(limitLength)
    const size = ea.measureText(text);
    console.log("In get Label Size");
  ...
  renderText():Dimensions {
    ...
    console.log(`width: ${labelSize.width}`);
